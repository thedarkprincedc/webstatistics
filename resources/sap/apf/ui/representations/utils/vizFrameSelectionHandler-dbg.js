...
(function() {
	...
	**/
	selectionHandlerPrototype.getSelectionInfoFromFilter = function(aFilters, aDataResponse) {
		var aDataPoint = [], oSelectionHandlerInstance = this;
		var sRequiredProperty = oSelectionHandlerInstance.oParameter.requiredFilters[0];
		if (oSelectionHandlerInstance.oParameter.requiredFilters[0]) {
			aDataResponse.forEach(function(dataRow) {
				aFilters.forEach(function(filterValue) {
					if (filterValue === dataRow[oSelectionHandlerInstance.oParameter.requiredFilters[0]]) {
						var selectionData = {
							data : {}
						};
						selectionData.data[sRequiredProperty] = filterValue;
