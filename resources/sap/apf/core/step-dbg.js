		...
		this.getFilterInformation = function(activeStep, index) {
			...
			function getFilterMappingInformation(){
				...
				oCoreApi.getMetadata(mappingRequest.service).done(function(filterMappingMetadata){
					var mappedPropertyMetadata = filterMappingMetadata.getPropertyMetadata(mappingRequest.entityType, mappedProperty);
				...
				filterMappingCache.responseData.forEach(function(data){
					if(displayOption === oLabelDisplayOptions.TEXT && textProperty){
						...
						mappedFilterValues.push({
							text : oCoreApi.getTextNotHtmlEncoded("keyAndTextSelection", [data[textProperty], data[mappedProperty]])
						});
					} else {
						mappedFilterValues.push({
				...
				mappedFilterValues = mappedFilterValues.sort(function(valueA, valueB){
					return valueA.text.localeCompare(valueB.text);
				});
				resolveInformationPromise(filterMappingDeferred, mappedProperty, mappedFilterValues, mappedPropertyLabel, mappingRequest, true);
